class MonthlySplitScheduleQuery < MonthlySplitSchedule::BaseQuery
  ...
  def send_bill_splits(only_send_every_n_hours : Int32 = 24)
    ...
    # specify how many months 'back' to go to look for other splits
    how_many_months_back = 24

    (0..how_many_months_back).each do |month_n|
      now_n_months_ago = now - month_n.month
      from = Time.utc(now_n_months_ago.year, now_n_months_ago.month, 1)
